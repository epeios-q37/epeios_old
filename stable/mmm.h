/*
  Header for the 'mmm' library by Claude L. Simon (epeios@epeios.org)
...
// Taille d'un bloc.
#define MMM_TAILLE_BLOC	( sizeof( mmm__indicateur ) )

// taille d'un descripteur
#define MMM_TAILLE_INDICATEUR	( sizeof( mmm__indicateur ) )
...
public:
	...
	mmm_multimemory_driver_( s &S )
	: S_( S ),
//	  MEMORY_DRIVER_( *(MEMORY_DRIVER_::s *)&S )
	  MEMORY_DRIVER_( S )
	{}
	...
	void Init( mmm_multimemory_ &Multimemory )
	{
		Liberer_();
	...
	mmm__descriptor Descriptor( void ) const
	{
		return S_.Descripteur;
	}
	//f Return the used multimemory.
	mmm_multimemory_ *Multimemory( void ) const
	{
		return Multimemoire_;
	}
	mmm_multimemory_driver_ &operator =( const mmm_multimemory_driver_ & )
	{
		ERRc();
...
private:
	...
	uym_basic_memory_ Memoire_;
// fonctions
	// Convertit un nombre en taille.
	uym__size NombreEnTaille_( mmm__nombre Nombre ) const
	{
		return Nombre * MMM_TAILLE_BLOC;
	}
	// Convertir une taille en nombre.
	mmm__nombre TailleEnNombre_( uym__size Size ) const
	{
		if ( !Size )
			return 0;
		else
	...
	void AllouerMemoire_( uym__size Size )
	{
	...
	void MemoireEcraser_(
		uym__position Destination,
		uym__position Source,
		uym__size Taille )
	{
	...
	void AjusterMemoireAvantDescripteur_( mmm__descriptor Descripteur )
	{
	...
	void AjusterMemoireApresDescripteur_( mmm__descriptor Descripteur )
	{
	...
	void Ecraser_(
		mmm__descriptor Destination,
		mmm__descriptor Source )
	{
	...
	void TesterEtAffecterLibre_( mmm__descriptor Descripteur )
	{
		if ( !S_.Libre
	...
	void Etendre_(
		...
		mmm__nombre Nombre )
	{
		mmm__descriptor Successeur = Successeur_( Descripteur );
		mmm__nombre NombreLibre = Nombre_( Successeur ) - ( Nombre - Nombre_( Descripteur ) ) + MMM_NOMBRE_INDICATEUR;
	...
	void Permuter_( mmm__descriptor Descripteur )
	{
		mmm__descriptor Libre = PredecesseurLibre_( Descripteur );
		mmm__descriptor Successeur = Successeur_( Descripteur );
		mmm__nombre NombreLibre = Nombre_( Libre );
	...
	void Fusionner_( mmm__descriptor Descripteur )
	{
		mmm__descriptor Libre = Successeur_( Descripteur ), LibreS = Successeur_( Libre );
	...
	{
		mmm__descriptor Successeur = Successeur_( Descripteur );
		mmm__nombre NombreLibre = Nombre_( Descripteur ) - Nombre + Nombre_( Successeur );
	...
	void Inserer_(
		...
		mmm__nombre Nombre )
	{
		mmm__nombre NombreLibre = Nombre_( Descripteur ) - Nombre;

		if ( NombreLibre > 1 )
		{
			mmm__descriptor Successeur;
	...
	void EcrireIndicateurLibre_(
		mmm__descriptor Descripteur,
		mmm__indicateur Indicateur )
	{
		MemoireEcrire_( (uym__data *)&Indicateur, MMM_TAILLE_INDICATEUR, Descripteur - MMM_TAILLE_INDICATEUR );
		MemoireEcrire_( (uym__data *)&Indicateur, MMM_TAILLE_INDICATEUR, Descripteur + NombreEnTaille_( Indicateur ) - MMM_TAILLE_INDICATEUR );
	...
	{
		mmm__indicateur Indicateur = Nombre | MMM_MASQUE_OCCUPATION | ( PredecesseurOccupe ? MMM_MASQUE_OCCUPATION_PREDECESSEUR : 0 );

		MemoireEcrire_( (uym__data *)&Indicateur, MMM_TAILLE_INDICATEUR, Descripteur - MMM_TAILLE_INDICATEUR );
	}
	/* Signale à la portion 'Descripteur' (occupée) qu'elle est précédée par une portion
	occupée */
	void SignalerPredecesseurOccupe_( mmm__descriptor Descripteur )
	{
		EcrireIndicateurOccupe_( Descripteur, Nombre_( Descripteur ), true );
	}
	/* Signale à la portion 'Descripteur' (occupée) qu'elle est précédée par une portion
	libre */
	void SignalerPredecesseurLibre_( mmm__descriptor Descripteur )
	{
	...
	mmm__indicateur IndicateurPredecesseur_( mmm__descriptor Descripteur ) const
	{
		mmm__indicateur Indicateur;

		MemoireLire_( Descripteur - 2 * MMM_TAILLE_INDICATEUR, MMM_TAILLE_INDICATEUR, (uym__data *)&Indicateur );

		return Indicateur;
	}
	/* Retourne le nombre de bloc du predecesseur de la portion de descripteur 'Descripteur'.
	N'a de sens que si le predecesseur est libre. */
	mmm__nombre NombrePredecesseur_( mmm__descriptor Descripteur ) const
	{
	...
	N'a de sens que si le predecesseur est libre. */
	uym__size TaillePredecesseur_( mmm__descriptor Descripteur ) const
	{
	...
	mmm__descriptor PredecesseurLibre_( mmm__descriptor Descripteur ) const
	{
	...
	void Liberer_( mmm__descriptor Descripteur )
	{
		...
		else
		{
			...
			if ( PredecesseurEstLibre_( Descripteur ) )
			{
				mmm__descriptor Predecesseur = PredecesseurLibre_( Descripteur );
	...
	mmm__descriptor Successeur_( mmm__descriptor Descripteur ) const
	{
	...
	bso__bool HorsLimite_( mmm__descriptor Descripteur ) const
	{
	...
	mmm__nombre NombreDisponibles_( mmm__descriptor Descripteur ) const
	{
		mmm__nombre Nombre = Nombre_( Descripteur );
		mmm__descriptor Successeur = Successeur_( Descripteur );
	...
	mmm__descriptor Allouer_( mmm__nombre Nombre )
	{
		mmm__descriptor Descripteur = 0;
		...
		if ( Nombre )
		{
			...
			{
				mmm__nombre NombreLibre = Nombre_( S_.Libre ) - Nombre;
...
public:
	struct s 
	{
		...
		uym_basic_memory_::s Memoire_;
		uym__size Capacite;
			// Descripteur à essayer lorsque l'on a besoin d'un emplacement libre.
		mmm__descriptor Libre;
	} &S_;
	mmm_multimemory_( s &S )
	: S_( S ),
	...
	void plug( MEMORY_DRIVER_ &Pilote = *(MEMORY_DRIVER_ *)NULL )
	{
	...
	mmm__descriptor Allocate( uym__size Size )
	{
	...
	is conserved. */
	mmm__descriptor Reallocate(
		mmm__descriptor Descriptor,
		uym__size Size )
	{
		mmm__nombre Nombre = TailleEnNombre_( Size );
		...
		else if ( HorsLimite_( Successeur_( Descriptor ) ) )
		{
			...
			{
				mmm__descriptor NouveauDescripteur = PredecesseurLibre_( Descriptor );
		...
		{
			mmm__descriptor NouveauDescripteur = Allouer_( Nombre );
	...
	void Free( mmm__descriptor Descriptor )
	{
		if ( Descriptor )
			Liberer_( Descriptor );
	}
	...
	// écrit 'Nombre' octets à la position 'Position'
/*	mmm__descriptor Descripteur( mmm__indice_bloc IndiceBloc )
	{
		if ( !IndiceBloc )
			return 0;
		else
	...
	mmm__indice_bloc IndiceBloc( mmm__descriptor Descripteur )
	{
		if ( !Descripteur )
			return 0;
		else
	...
	mmm_multimemory_ &operator =( const mmm_multimemory_ &Op )
	{
	...
	uym__size Size( mmm__descriptor Descriptor ) const
	{
		if ( !Descriptor )
			return 0;
		else
...
AUTO( mmm_multimemory )

inline void mmm_multimemory_driver_::Liberer_( void )
{
	if ( S_.Descripteur )
		Multimemoire_->Free( S_.Descripteur );
}

inline void mmm_multimemory_driver_::MDRRead(
	mdr__position Position,
	mdr__bsize Amount,
	mdr__data *Buffer )
{
...
// lit à partir de 'Position' et place dans 'Tampon' 'Nombre' octets;
inline void mmm_multimemory_driver_::MDRWrite(
	const mdr__data *Buffer,
	mdr__bsize Amount,
	mdr__position Position )
{
...
// écrit 'Nombre' octets à la position 'Position'
inline void mmm_multimemory_driver_::MDRAllocate( mdr__size Size )
{
	S_.Descripteur = Multimemoire_->Reallocate( S_.Descripteur, Size );
}
// alloue 'Capacite' octets

inline void mmm_multimemory_driver_::MDRSynchronize( void )
{
	if ( Multimemoire_ )
