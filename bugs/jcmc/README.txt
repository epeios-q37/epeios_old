You will find at http://cvs.savannah.gnu.org/viewvc/epeios/bugs/jcmc/?root=epeios a test case to demonstrate the crash which happends when the 'malloc(...)' function is called within a Java native componenent compiled with g++ under Cygwin. There are 5 files :
	...
	- jcmc.java : the Java source code which calls the native omponenent.
	
Given that you have installed 'Cygwin' with 'make' and 'g++', and also the JDK and JRE, you have to adapt the first line of the 'Makefile' file to set the 'jdk' variable to the directory where your JDK is installed. Then, you have to go with 'Cygwin' to the directory containing all the above files, and launch the 'make' command.
...
If you open the 'jcmc.cpp' file, which is the source code of the Java native component, you will see that there is only a 'malloc(1)' call, which shoudn't behave like that.
