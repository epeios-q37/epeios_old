/*
	'xmllpr' library by Claude SIMON (csimon at zeusw dot org)
	Requires the 'xmllpr' header file ('xmllpr.h').
...
public:
	xmllprtutor( void )
	: ttr_tutor( XMLLPR_NAME )
	{
#ifdef XMLLPR_DBG
		Version = XMLLPR_VERSION "\b\bD $";
#else
		Version = XMLLPR_VERSION;
#endif
		Owner = XMLLPR_OWNER;
		Date = "$Date$";
	}
	virtual ~xmllprtutor( void ){}
};
...
ERRBegin
	...
	while ( !Flow.EOX() ) {
		...
		case OpeningTag:
			...
			Callback.XMLLPRTag( Prefix, Name );
		...
		case Attribute:
			...
			Callback.XMLLPRAttribute( Prefix, Name, Value );
...
ERRErr
	if ( ( ERRMajor == err::itn ) && ( ERRMinor == err::iBeam ) ) {
			Callback.XMLLPRError( Flow.Line(), Flow.Column() );
			ERRRst();
	}
ERREnd
ERREpilog
...
class xmllprpersonnalization
: public xmllprtutor
{
public:
	xmllprpersonnalization( void )
	{
...
static xmllprpersonnalization Tutor;

ttr_tutor &XMLLPRTutor = Tutor;
