/*
	'xmldcm' library by Claude SIMON (csimon at zeusw dot org)
	...
	value_ &Value ) const
{
	...
	while( Row != NONE ) {
		Value.Append( TaggedValue( Row ).Value );
		Row = Next( Row );
	}
}
	...
	document_filler__ &XMLF )
{
	...
	while( ValueRow != NONE ) {
		if ( Tag( TaggedValue( ValueRow ).TagRow() ).Type() == tAttribute ) {
			XMLF.PutAttribute( Tag( TaggedValue( ValueRow ).TagRow() ).Name, TaggedValue( ValueRow ).Value );
		} else if ( !Return )
			Return = true;
...
void document_filler__::Put( const document_ &XMLD ) 
{
ERRProlog
	dtr::browser__<value_row__> Browser;
	ctn::E_CMITEMt( tagged_value_, value_row__ ) TaggedValue;
...
ERRBegin
	...
	while( Browser.Position() != NONE ) {
		...
		case dtr::bChild:
			if ( ( ( TagRow = TaggedValue( Browser.Position() ).TagRow() ) == NONE )
			     || !Tag( TagRow ).IsAttribute() ) {
				PushTag( Tag( TaggedValue( XMLD.Parent( Browser.Position() ) ).TagRow() ).Name );
				WriteAttributes_( XMLD, Browser.Position(), *this );
				PutValue( TaggedValue( Browser.Position() ).Value );
			}
			break;
		case dtr::bParent:
